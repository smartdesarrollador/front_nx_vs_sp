    ...
    <div className="bg-white dark:bg-gray-900 shadow-sm rounded-xl overflow-hidden grid grid-cols-1 md:grid-cols-[280px_1fr] min-h-[600px]">
      ...
      <aside className="bg-gray-50 dark:bg-gray-800 p-6 flex flex-col gap-6">
        ...
        <CVContactSection
          ...
          phone={phone}
        />
        <CVSkillsSection skills={cv.skills} title={labels.skills} variant="bars" />
      ...
      <main className="p-6 md:p-8 flex flex-col gap-8">
        {cv.professional_summary && (
          <section>
            <h2 className="text-lg font-semibold text-gray-900 dark:text-white mb-3 pb-2 border-b border-gray-200 dark:border-gray-700" style={accentStyle}>
              {labels.summary}
            </h2>
                    ...
                    {p.technologies.length > 0 && (
                      <div className="flex flex-wrap gap-1 mt-1">
                        {p.technologies.map((t, j) => (
                          <span key={j} className="text-xs bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 px-1.5 py-0.5 rounded">{t}</span>
                        ))}
                      </div>
                    )}
                  </div>
